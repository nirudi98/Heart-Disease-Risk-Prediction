...
if __name__ == '__main__':
    ...
    heart = heart.rename(
        columns={"chest pain type": "chest_pain", "max heart rate": "maxHeartRate", "resting bp s": "blood_pressure", "fasting blood sugar": "blood_sugar",
                 "ca": "vessels", "chol": "cholesterol", "sex": "gender", "target": "heart_status", "exercise angina": "exercise_angina"})

    print("\n")

    # drop unwanted columns
    heart = heart.drop(['resting ecg', 'oldpeak', 'ST slope'], axis=1)
    ...
    # scaler_model3 = 'scaler3.sav'
    # file_path = 'D:/python/Cardio/model_three/baseModels/'
    # with open(file_path + scaler_model3, 'wb') as file:
    ...
    STACK_DL_meta = pickle.load(open('D:/python/Cardio/model_three/baseModels/meta_model_DL3.pkl', 'rb'))
    DL_models = ann_models.load_all_models(3)

    scaler2 = pickle.load(open('D:/python/Cardio/model_three/baseModels/scaler3.sav', 'rb'))
    array = [[57, 1, 0, 130, 131, 0, 115, 1]]
    array = np.array(array)

    scale = scaler2.transform(array)

    print("HeartModelThree Home dataset, new prediction using STACK ML :", STACK_ML.predict(scale))
    probs_ml = STACK_ML.predict_proba(scale)
    ...
    print("HeartModelThree Home dataset, new prediction using STACK DL :", prediction)
    ...
    print("HeartModelThree Home dataset, average prob", avg_prob[:, 1])
    ...
    model_results.to_csv(path+"model 3 COPY results")
